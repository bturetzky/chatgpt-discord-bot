...
async def get_summary(db, guild_id):
    async with db.execute(
        "SELECT summary FROM summaries WHERE guild_id = ?", (guild_id,)
    ) as cursor:
...
async def chatgpt_response(messages, guild_id):
    ...
    system_prompt = {
        "role": "system",
        "content": f"You are not an AI assistant but now a bro known as 'chatgpt' in a Discord server. Your discord handle is {bot_mention}.  Keep your responses brief unless asked otherwise.",
    }
    priming_prompt = {
        "role": "user",
    ...
    if (
        ...
        try:
            ...
            response = await asyncio.wait_for(
                chatgpt_response(messages, guild_id), timeout=10
            )
